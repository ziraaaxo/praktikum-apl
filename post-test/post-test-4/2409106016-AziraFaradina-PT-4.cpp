...
string cek_strength(const string &, int); // fungsi overloading
...
// fungsi mengecek kekuatan password
string cek_strength(const string &pass) {
    int len = pass.length();
    return cek_strength(pass, len);
}

// fungsi overloading: terima password + panjang
string cek_strength(const string &pass, int len) {
...
void show_pass_data(const user &u, int i) {
    const data_password &p = u.daftar_pass[i];
...
void update_pass(user &u) {
    ...
    data_password &p = u.daftar_pass[idx - 1];
